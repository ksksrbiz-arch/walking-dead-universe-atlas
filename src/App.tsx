...
export default function App(){
 ...
 const selectEpisode=(id:string)=>{const raw=atlasData.episodes.find((e:any)=>e.id===id) as any;setYearForEpisode(raw);setSelectedEpisode(id);setSelectedLocation(null);setSelectedCharacter(null);setSelectedConnection(null);setJourneyMapMode(false);setView("timeline");setSheet("open")};
 ...
 const selectAtlasEpisode=(id:string)=>{const raw=atlasData.episodes.find((e:any)=>e.id===id) as any;setYearForEpisode(raw);setSelectedEpisode(id);setSelectedLocation(null);setSelectedCharacter(null);setSelectedConnection(null);setJourneyMapMode(false);setView("map");setSheet("open");focusEpisodeGeography(raw)};
 ...
 const goView=(v:View)=>{setView(v);setSelectedLocation(null);setSelectedEpisode(null);setSelectedCharacter(null);setSelectedConnection(null);setJourneyMapMode(false);setSheet("open")};
 ...
 const isCharacterJourneyContext=journeyMapMode&&characterJourneyLocationIds.has(l.id);const iconSize=isSelected?20:18;const iconHalf=iconSize/2;return <g key={l.id} data-location-id={l.id} className={"marker"+(isSelected?" selected":"")+(isEpisodeContext?" episodeContext":"")+(isConnectionContext?" connectionContext":"")+(isCharacterJourneyContext?" characterJourneyContext":"")} transform={`translate(${p.x} ${p.y})`} role="button" tabIndex={0} aria-label={`Open ${l.name} location`} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();selectLocation(l)}}} onPointerUp={e=>{if(!drag.current.moved){e.stopPropagation();selectLocation(l)}}}>
    ...
    <section className={`contentPanel ${sheet} ${selectedLoc||selectedEp||selectedCharacter||selectedConnection?"hasDetail":""}`}>
      ...
      {selectedLoc?<LocationDetail location={selectedLoc} onEpisode={selectEpisode} onCharacter={selectCharacter} onLocation={selectLocation} onConnection={selectConnection}/>:selectedEp?<EpisodeDetail episode={selectedEp} onLocation={selectLocation} onEpisode={selectEpisode} onCharacter={selectCharacter} onConnection={selectConnection}/>:selectedCharacter?<CharacterDetail characterId={selectedCharacter} onEpisode={selectEpisode} onLocation={selectLocation} onCharacter={selectCharacter} onConnection={selectConnection} onJourney={focusCharacterJourney}/>:selectedConnection?<ConnectionDetail connectionId={selectedConnection} onCharacter={selectCharacter} onLocation={selectLocation} onEpisode={selectEpisode} onConnection={selectConnection}/>:view==="map"?<MapContent locations={mapLocations} onSelect={selectLocation}/>:view==="timeline"?<TimelineContent episodes={episodes} onEpisode={selectEpisode} onUniverseEvent={selectUniverseEvent}/>:view==="people"?<PeopleContent onCharacter={selectCharacter} onLocation={id=>{const l=atlasData.locations.find(x=>x.id===id);if(l)selectLocation(l)}} onEpisode={selectEpisode} onConnection={selectConnection} focusEntity={peopleFocusEntity}/>:<GuideContent errors={dataErrors} onView={goView}/>}
      </ErrorBoundary>
    </section>
...
function ConnectionDetail({connectionId,onCharacter,onLocation,onEpisode,onConnection}:{connectionId:string;onCharacter:(id:string)=>void;onLocation:(l:Location)=>void;onEpisode:(id:string)=>void;onConnection:(id:string)=>void}){
 ...
 return <div className="contentScroll">
  ...
  <EntityGraphView heading="TRAVERSE THIS LINK" root={"connection:"+connectionId} onCharacter={onCharacter} onLocation={id=>{const l=atlasData.locations.find(x=>x.id===id);if(l)onLocation(l)}} onEpisode={onEpisode} onConnection={onConnection}/>
 </div>;
}
function EpisodeDetail({episode,onLocation,onEpisode,onCharacter,onConnection}:{episode:any;onLocation:(l:Location)=>void;onEpisode:(id:string)=>void;onCharacter:(id:string)=>void;onConnection:(id:string)=>void}){
...
function PeopleContent({onCharacter,onLocation,onEpisode,onConnection,focusEntity:focusEntityProp}:{onCharacter:(id:string)=>void;onLocation:(id:string)=>void;onEpisode:(id:string)=>void;onConnection:(id:string)=>void;focusEntity?:string|null}){
 const [query,setQuery]=useState("");
 const [focusEntity,setFocusEntity]=useState<string|null>(focusEntityProp??null);
 useEffect(()=>{if(focusEntityProp)setFocusEntity(focusEntityProp)},[focusEntityProp]);
 const q=query.trim().toLowerCase();
 ...
 return <div className="contentScroll">
  ...
  <EntityGraphView heading="EXPLORE CONNECTIONS" onCharacter={onCharacter} onLocation={onLocation} onEpisode={onEpisode} onConnection={onConnection}/>
  ...
  <div className="sectionTitle">FACTIONS <span>{atlasData.factions.length}</span></div><div className="miniTags">{atlasData.factions.map((f:any)=><button className="entityTagButton" key={f.id} onClick={()=>setFocusEntity("faction:"+f.id)}>{f.name}</button>)}</div><div className="sectionTitle">COMMUNITIES <span>{atlasData.communities.length}</span></div><div className="miniTags">{atlasData.communities.map((c:any)=><button className="entityTagButton" key={c.id} onClick={()=>setFocusEntity("community:"+c.id)}>{c.name}</button>)}</div>{focusEntity&&<><button className="entityFocusClear" onClick={()=>setFocusEntity(null)}>CLEAR ENTITY FOCUS</button><EntityGraphView heading="ENTITY RELATIONSHIPS" root={focusEntity} onCharacter={onCharacter} onLocation={id=>{const l=atlasData.locations.find(x=>x.id===id);if(l)onLocation(l)}} onEpisode={onEpisode} onConnection={onConnection}/></>}
