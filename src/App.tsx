...
function LocationDetail({location,onEpisode}:{location:Location;onEpisode:(id:string)=>void}){
 ...
 return <div className="contentScroll">
  <div className="entityHero" style={{"--accent":meta.color} as CSSProperties}>{placeMedia?.image&&<img src={atlasImageUrl(placeMedia.image,1200)} srcSet={atlasImageSrcSet(placeMedia.image)} sizes="(max-width: 699px) 92vw, 470px" loading="eager" decoding="async" alt="" className="entityArt"/>}<div className="entityHeroCopy"><span>{meta.short} · {location.year}</span><h3>{location.name}</h3><p>{location.type} · {location.certainty}</p></div></div>
...
function MapContent({locations,onSelect}:{locations:Location[];onSelect:(l:Location)=>void}){
 return <div className="contentScroll">
  ...
  <div className="cards">{locations.map(l=>{const pm=(atlasData as any).media?.places?.[l.id];return <button className="entityCard locationCard" key={l.id} onClick={()=>onSelect(l)}>{pm?.image&&<img src={atlasImageUrl(pm.image,720)} srcSet={atlasImageSrcSet(pm.image,[360,540,720])} sizes="180px" loading="lazy" decoding="async" alt="" className="cardArt"/>}<span><small>{SERIES_BY_ID[l.seriesId]?.short} · {l.year}</small><b>{l.name}</b><em>{l.type} · {l.certainty}</em></span><Icon name="chevron"/></button>})}</div>
 </div>
}
...
function TimelineContent({episodes,onEpisode}:{episodes:any[];onEpisode:(id:string)=>void}){
 return <div className="contentScroll">
  ...
  <div className="timelineList episodeList">{episodes.map((e:any)=>{const em=(episodeMedia as any).episodes?.[e.id];return <button className={em?.image?"episodeRow mediaRow hasMedia":"episodeRow mediaRow"} key={e.id} onClick={()=>onEpisode(e.id)}>{em?.image&&<span className="rowMedia"><img src={atlasImageUrl(em.image,240)} srcSet={atlasImageSrcSet(em.image,[160,240,360])} sizes="80px" loading="lazy" decoding="async" alt="" className="rowThumb"/></span>}<strong>{e.start||"?"}</strong><span><small>{SERIES_BY_ID[e.seriesId]?.short} · S{String(e.seasonId).slice(-2)}E{String(e.episodeNumber).padStart(2,"0")}</small><b>{e.title}</b><em>{e.certainty} · {e.precision}</em></span><Icon name="chevron"/></button>})}</div>
 </div>
}
