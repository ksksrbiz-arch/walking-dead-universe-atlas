...
function ConnectionDetail({connectionId,onCharacter,onLocation,onEpisode,onConnection,onCommunity,onFaction}:{connectionId:string;onCharacter:(id:string)=>void;onLocation:(l:Location)=>void;onEpisode:(id:string)=>void;onConnection:(id:string)=>void;onCommunity:(id:string)=>void;onFaction:(id:string)=>void}){
 ...
 return <div className="contentScroll">
  ...
  <MiniTimeline items={episodes.map((e:any)=>({id:e.id,start:Number(e.timelineStart??e.timelineEnd??0),end:Number(e.timelineEnd??e.timelineStart??0),title:e.title,color:SERIES_BY_ID[e.seriesId]?.color}))} onSelect={onEpisode} emptyLabel="No episode evidence anchored to universe time yet."/>
...
function PeopleContent({onCharacter,onLocation,onEpisode,onConnection,onCommunity,onFaction,focusEntity:focusEntityProp,onFocusEntity}:{onCharacter:(id:string)=>void;onLocation:(id:string)=>void;onEpisode:(id:string)=>void;onConnection:(id:string)=>void;onCommunity:(id:string)=>void;onFaction:(id:string)=>void;focusEntity?:string|null;onFocusEntity:(value:string|null)=>void}){
 const [query,setQuery]=useState("");
 ...
 return <div className="contentScroll">
  ...
  <div className="sectionTitle">FACTIONS <span>{atlasData.factions.length}</span></div><div className="miniTags">{atlasData.factions.map((f:any)=><button className="entityTagButton" key={f.id} onClick={()=>onFaction(f.id)}>{f.name}<small className="entityTagMeta">{f.certainty}</small></button>)}</div><div className="sectionTitle">COMMUNITIES <span>{atlasData.communities.length}</span></div><div className="miniTags">{atlasData.communities.map((c:any)=><button className="entityTagButton" key={c.id} onClick={()=>onCommunity(c.id)}>{c.name}<small className="entityTagMeta">{[c.type?.replaceAll("-"," "),c.certainty].filter(Boolean).join(" · ")}</small></button>)}</div>{focusEntity&&<><button className="entityFocusClear" onClick={()=>onFocusEntity(null)}>CLEAR ENTITY FOCUS</button><EntityGraphView heading="ENTITY RELATIONSHIPS" root={focusEntity} onCharacter={onCharacter} onLocation={id=>{const l=atlasData.locations.find(x=>x.id===id);if(l)onLocation(l)}} onEpisode={onEpisode} onConnection={onConnection} onCommunity={onCommunity} onFaction={onFaction}/></>}
