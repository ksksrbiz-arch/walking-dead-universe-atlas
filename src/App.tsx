...
function PeopleContent({onCharacter,onLocation,onEpisode,onConnection,onCommunity,onFaction,focusEntity:focusEntityProp}:{onCharacter:(id:string)=>void;onLocation:(id:string)=>void;onEpisode:(id:string)=>void;onConnection:(id:string)=>void;onCommunity:(id:string)=>void;onFaction:(id:string)=>void;focusEntity?:string|null}){
 const [query,setQuery]=useState("");
 const [focusEntity,setFocusEntity]=useState<string|null>(focusEntityProp??null);
 useEffect(()=>{setFocusEntity(focusEntityProp??null)},[focusEntityProp]);
 const q=query.trim().toLowerCase();
