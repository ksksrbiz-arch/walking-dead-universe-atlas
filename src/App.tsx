...
export default function App(){
 ...
 useEffect(()=>{
   const onKey=(e:KeyboardEvent)=>{
     ...
     if(e.key==="Escape"){setSearchOpen(false);setSelectedLocation(null);setSelectedEpisode(null);setSelectedCharacter(null);setSelectedConnection(null)}
 ...
 const selectLocation=(l:Location)=>{ const focusStarted=performance.now();
   ...
   if(Number(l.year)>0)setYear(Number(l.year));setSelectedLocation(l.id);setSelectedEpisode(null);setSelectedCharacter(null);setSelectedConnection(null);setView("map");setSheet("open");
 ...
 const selectEpisode=(id:string)=>{const raw=atlasData.episodes.find((e:any)=>e.id===id) as any;setYearForEpisode(raw);setSelectedEpisode(id);setSelectedLocation(null);setSelectedCharacter(null);setSelectedConnection(null);setView("timeline");setSheet("open")};
 ...
 const selectAtlasEpisode=(id:string)=>{const raw=atlasData.episodes.find((e:any)=>e.id===id) as any;setYearForEpisode(raw);setSelectedEpisode(id);setSelectedLocation(null);setSelectedCharacter(null);setSelectedConnection(null);setView("map");setSheet("open");focusEpisodeGeography(raw)};
 ...
 const goView=(v:View)=>{setView(v);setSelectedLocation(null);setSelectedEpisode(null);setSelectedCharacter(null);setSelectedConnection(null);setSheet("open")};
 ...
 return <div className="app">
  <header className="topbar">
   <button className="brand" onClick={()=>{setView("map");setSelectedLocation(null);setSelectedEpisode(null);setSelectedCharacter(null);setSelectedConnection(null)}} aria-label="Return to atlas map">
   ...
   <section className={"map view-"+view+(selectedLocation||selectedEpisode||selectedCharacter||selectedConnection?" detailOpen":"")} aria-label="Interactive Walking Dead Universe map">
    ...
    <div className="mapSurface">
     <svg ref={mapSvgRef} viewBox="0 0 1000 600" preserveAspectRatio="xMidYMid slice" className={isDragging?"dragging":""} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onWheel={wheel}>
      ...
      <g className="markers">{locations.filter(hasMapCoordinates).map(l=>{const p=project(l.lat,l.lng),meta=SERIES_BY_ID[l.seriesId];const isSelected=selectedLocation===l.id;const isEpisodeContext=episodeContextLocationIds.has(l.id);
 const isConnectionContext=connectionContextLocationIds.has(l.id);const iconSize=isSelected?20:18;const iconHalf=iconSize/2;return <g key={l.id} data-location-id={l.id} className={"marker"+(isSelected?" selected":"")+(isEpisodeContext?" episodeContext":"")+(isConnectionContext?" connectionContext":"")} transform={`translate(${p.x} ${p.y})`} role="button" tabIndex={0} aria-label={`Open ${l.name} location`} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();selectLocation(l)}}} onPointerUp={e=>{if(!drag.current.moved){e.stopPropagation();selectLocation(l)}}}>
    ...
    <section className={`contentPanel ${sheet} ${selectedLoc||selectedEp||selectedCharacter||selectedConnection?"hasDetail":""}`}>
      ...
      <div className={`panelHeader ${selectedLoc||selectedEp||selectedCharacter||selectedConnection?"detailHeader":""}`}>
       ...
       {(selectedLoc||selectedEp||selectedCharacter)&&<button className="closePanel" onClick={()=>{setSelectedLocation(null);setSelectedEpisode(null);setSelectedCharacter(null);setSelectedConnection(null)}} aria-label="Close details"><Icon name="close"/></button>}
      </div>
...
function CharacterDetail({characterId,onEpisode,onLocation,onCharacter,onConnection}:{characterId:string;onEpisode:(id:string)=>void;onLocation:(l:Location)=>void;onCharacter:(id:string)=>void;onConnection:(id:string)=>void}){
 const [runtimeEpisodeIds,setRuntimeEpisodeIds]=useState<string[]|null>(null);
