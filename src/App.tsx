...
function EpisodeDetail({episode,onLocation,onEpisode,onCharacter,onConnection,onCommunity,onFaction}:{episode:any;onLocation:(l:Location)=>void;onEpisode:(id:string)=>void;onCharacter:(id:string)=>void;onConnection:(id:string)=>void;onCommunity:(id:string)=>void;onFaction:(id:string)=>void}){
 ...
 const fandomEpisodeImage=fandomEntityImage("episodes",episode.id,episode.title);
 const heroImage=media?.image||fandomEpisodeImage;
...
function MapContent({locations,onSelect}:{locations:Location[];onSelect:(l:Location)=>void}){
 ...
 return <div className="contentScroll">
  ...
  <div className="cards">{mappedLocations.map(l=>{const pm=(atlasData as any).media?.places?.[l.id];const specificImage=pm?.image||fandomEntityImage("locations",l.id,l.name);const image=specificImage||(atlasData as any).media?.series?.[l.seriesId]?.keyArt;const isGenericFallback=Boolean(image)&&!specificImage;return <button className="entityCard locationCard" key={l.id} onClick={()=>onSelect(l)}>{image&&<img src={atlasImageUrl(image,720)} onError={e=>onAtlasImageError(e,image)} srcSet={atlasImageSrcSet(image,[360,540,720])} sizes="180px" loading="lazy" decoding="async" alt="" className="cardArt"/>}<span><small>{SERIES_BY_ID[l.seriesId]?.short} · {l.year}</small><b>{l.name}</b><em>{l.type} · {l.certainty}</em></span><i className={isGenericFallback?"mediaIndicator fallback":"mediaIndicator verified"} aria-label={isGenericFallback?"No verified image for this location; showing generic series art":"Location-specific image"}/><Icon name="chevron"/></button>})}</div>
...
function TimelineContent({episodes,onEpisode,onUniverseEvent,onConnections,onYearChange,currentYear}:{episodes:any[];onEpisode:(id:string)=>void;onUniverseEvent:(event:any)=>void;onConnections:()=>void;onYearChange:(year:number)=>void;currentYear:number}){
 ...
 return <div className="contentScroll">
  ...
  <button className={"timelineSecondaryToggle "+(showWebisodes?"active":"")} onClick={()=>setShowWebisodes(v=>!v)}><span>SECONDARY LAYER</span><b>{showWebisodes?"HIDE":"SHOW"} WEBSERIES</b><small>Release chronology · 63 installments · story placement not forced where exact in-universe dates are unavailable.</small><Icon name="chevron"/></button>
