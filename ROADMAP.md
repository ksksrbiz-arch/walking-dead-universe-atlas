...
- [x] Verify the standalone Walking Dead API endpoints and schema — dead (404 on every path; `data/enrichment/sourceRegistry.json` id `walking-dead-api`)
- [x] Verify API-TWD-Characters endpoint/repository and schema — an unhosted repo, not a running service; not worth pursuing (`sourceRegistry.json` id `api-twd-characters`)
