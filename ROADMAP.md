...
- [x] Scope Wikidata for entity reconciliation before building against it — evaluated via `wbsearchentities` + the public SPARQL endpoint; not worth building reconciliation infrastructure on right now (coverage is flagship-show-only and partial, comic/TV-canon items are ambiguous by name the same way Fandom pages already were, and it has nothing on this atlas's actual differentiator — in-universe fictional geography). Full findings and evidence in `data/enrichment/sourceRegistry.json` id `wikidata` (now `evaluated-low-priority`). Deferred, not pursued.
- [ ] Add external IDs and entity reconciliation — deferred pending a stronger source than Wikidata (see above), or a concrete need for one
